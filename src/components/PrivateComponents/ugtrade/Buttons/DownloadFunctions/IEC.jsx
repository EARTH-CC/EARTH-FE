...
const downloadData = (tableData) => {
  ...
  sheets.forEach((sheet) => {
    ...
    }: Report on IEC Materials (${sheet.name})`;
    ...
    filteredData.forEach((data) => {
      const rowData = [
        data.report_date,
        data.title_of_iec_material,
        data.no_of_copies_distributed,
    ...
    const totalAreaFormula = `SUM(C6:C${filteredData.length + 5})`;
    worksheet.getCell(`C${filteredData.length + 7}`).value = {
      formula: totalAreaFormula,
    };
    worksheet.getCell(`C${filteredData.length + 7}`).font = {
      bold: true,
    };
    worksheet.getCell(`C${filteredData.length + 7}`).alignment = {
      horizontal: "center",
    };
    // Set the total area cell format
    const totalAreaCell = worksheet.getCell(`C${filteredData.length + 7}`);
    totalAreaCell.numFmt = "0.00";

    // Add "Total" text in the cell next to "Area in Hectares (ha)"
    worksheet.getCell(`B${filteredData.length + 7}`).value = "Total";
    worksheet.getCell(`B${filteredData.length + 7}`).font = {
      bold: true,
    };
    worksheet.getCell(`B${filteredData.length + 7}`).alignment = {
      horizontal: "right",
    };
  ...
  const filename = `IEC_Report_${tableData[0].report_date}.xlsx`;
