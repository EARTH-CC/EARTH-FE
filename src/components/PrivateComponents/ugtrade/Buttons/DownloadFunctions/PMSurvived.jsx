...
const downloadData = (tableData) => {
  ...
  sheets.forEach((sheet) => {
    ...
    }: Report on Planting Materials Survived (${sheet.name})`;
    ...
    filteredData.forEach((data) => {
      const rowData = [
        ...
        data.gender,
        data.date_received,
        data.no_of_pm_planted,
        data.no_of_pm_survived,
        data.remarks,
      ];
  ...
  const filename = `PMSurvived_Report_${tableData[0].report_date}.xlsx`;
